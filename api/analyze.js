...
【維持原文規則 — 最高優先，優先於所有其他規則】

⚠️ 此規則優先層級最高，不得被任何其他規則覆蓋或例外處理。

以下類型的問題，optimized 欄位必須完整複製原始回答的每一個字，一字不改、一字不增、一字不刪，也不加鉤子、不加連結、不加標點修改：
- 退貨、換貨、退款相關問題（含「我要退貨」「我要換貨」「退款」等關鍵字）
- 客服聯絡方式、客服流程
- 具體政策說明（退換貨政策、保固、條款、規範）
- 含有電話號碼、信箱、LINE 帳號等聯絡資訊的回答
- 含有具體時限（如「7日內」「7-14個工作天」）、金額、法規說明的回答
- 行政流程說明（如申請步驟、審核流程）

這類問題的原始回答是官方政策內容，任何修改都可能造成誤解或法律疑慮。
optimized 欄位 = 原始回答，不多不少，原文照錄。
hook_link 欄位填空字串 ""。
...
嚴禁以下行為：
❌ 從其他題目的回答中借用任何文字、知識點、菌株名稱、症狀描述、數據、列表項目
❌ 新增原始回答中未出現的任何資訊（包括補充說明、延伸建議）
❌ 查詢或補充網路資料
❌ 調換題目順序或跳過任何題目（每題都必須輸出，idx順序不變）
❌ 增加或刪除原文列出的清單項目（菌株名稱、數字、步驟等必須一模一樣）
❌ 把其他題有、但本題沒有提到的菌株名稱或效果寫進本題
...
逐句自我檢查（必須執行）：優化完每一題後，逐句比對優化版與原始回答，確認：「優化版裡的每一個名詞、數字、清單項目，是否都出現在這題自己的原始回答中？」若有任何一項不是出自本題原文 → 立即刪除，不得保留。
...
2. 在全部 FAQ 題目中，逐題搜尋是否存在「使用者順著鉤子回答後，能真正對應到的下一題」
   - 判斷標準：該題的問題主題，必須直接對應到鉤子問句所指向的情境或關鍵字
   - ❌ 禁止：鉤子問「你有便秘嗎？」但 hook_link 填一題問「益生菌怎麼挑」→ 主題不符，不可填
   - ✅ 正確：鉤子問「你有便秘嗎？」→ hook_link 填真正有一題問「益生菌可以改善便秘嗎？」的那一列

3. hook_link 填寫規則（嚴格遵守）：
   - 輸入的 FAQ 一定有明確題號（Q1、Q2、Q3⋯），hook_link 一律填題號格式，例如「Q5」
   - 找不到對應題目 → 列入缺題建議，hook_link 填缺題建議的編號（如「缺1」）
   - 維持原文類題目（退換貨、政策等）→ hook_link 填空字串 ""
   - ❌ 絕對禁止：填一個「看起來相關」但實際上主題不符的題號，寧可填「缺N」也不亂填
   - ❌ 絕對禁止：hook_link 填自己這一題的題號（如 Q2 的題目，hook_link 不可以填「Q2」）。鉤子的目的是銜接「下一題」，指向自己毫無意義。
...
function validateHookLinks(json) {
  ...
  json.items = json.items.map((item) => {
    const qNum = (item.idx ?? 0) + 1; // Q number = idx + 1
    const hl = (item.hook_link || '').trim();

    if (!hl) return item;

    // Detect self-reference: "QN" where N == qNum, or "第N列" where N == qNum
    const qMatch = hl.match(/^Q(\d+)$/i);
    const rowMatch = hl.match(/^第(\d+)列$/);
    const isSelf = (qMatch && parseInt(qMatch[1], 10) === qNum) ||
                   (rowMatch && parseInt(rowMatch[1], 10) === qNum);
    ...
    if (isSelf) {
      ...
      json.gaps.push({
        ...
        cat: item.cat || '',
        suggest_q: `（待補）接住 Q${qNum} 鉤子方向的下一題`,
        suggest_c: '請根據本題鉤子問句設計對應回答，推進使用者意圖。',
        intent: item.intent === '低' ? '中' : '高',
        reason: `Q${qNum} 的 hook_link 原本填了自己（${hl}），屬於自我循環，已自動轉為缺題 ${gapId}。請補充能接住此題鉤子方向的新問題。`
      });
    }
